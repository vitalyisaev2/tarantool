...
local function bootstrap()
    -- quick check that data dictionary is empty
    if box.space._schema:get('version') ~= nil then
        return
    end
...
local function upgrade_index_options_to_1_6_8()
    ...
    for _, def in box.space._index:pairs() do
        if type(def[5]) == 'number' then
            ...
            print('orig def', json.encode(def), json.encode(new_def))
...
local function upgrade()
    box.cfg{}
    local version = box.space._schema:get{'version'} or {'version', 1, 6, 0}
